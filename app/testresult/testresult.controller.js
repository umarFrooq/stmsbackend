...
const createTestResultHandler = catchAsync(async (req, res) => {
  ...
  const testResult = await testResultService.createTestResult(req.body, userId, req.files);
...
const updateTestResultHandler = catchAsync(async (req, res) => {
  ...
  const testResult = await testResultService.updateTestResultById(
    ...
    req.body,
    userId,
    req.files // Pass the path of the new image, or null to remove, or undefined if not changing
  );
  res.send(testResult);
});
