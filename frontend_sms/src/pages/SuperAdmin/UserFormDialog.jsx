...
const UserFormDialog = ({ open, onClose, user, onSubmit, availableRoles = [] }) => {
  ...
  const validationSchema = Yup.object().shape({
    ...
    confirmPassword: Yup.string().when(['isEditing', 'password'], ([isEditingActual, password], schema) => {
      if (!isEditingActual || (password && password.length > 0)) {
        return schema
          .oneOf([Yup.ref('password'), null], 'Passwords must match')
          .required('Confirm password is required');
      }
  ...
  const handleSubmit = async (values, { setSubmitting }) => {
    ...
    if (isEditing) {
      if (values.fullname !== user?.fullname) submissionValues.fullname = values.fullname;
      if (values.role !== user?.role) submissionValues.role = values.role;
      const initialBranchId = user?.branchId || '';
      if (values.branchId !== initialBranchId) submissionValues.branchId = values.branchId;
      if (values.status !== user?.status) submissionValues.status = values.status;
    } else {
